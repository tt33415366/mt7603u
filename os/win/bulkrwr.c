...
--*/
{
   ...
   DBGPRINT(RT_DEBUG_TRACE, ("BulkUsb_DispatchReadWrite - begins\n"));

    if(deviceExtension->DeviceState != USBWorking) {

       DBGPRINT(RT_DEBUG_TRACE, ("Invalid device state\n"));
   ...
   DBGPRINT(RT_DEBUG_TRACE, ("Waiting on the IdleReqPendEvent\n"));
    ...
    if(fileObject && fileObject->FsContext) {
        ...
        if((UsbdPipeTypeBulk != pipeInformation->PipeType) &&
           ...
           DBGPRINT(RT_DEBUG_TRACE, ("Usbd pipe type is not bulk or interrupt\n"));
       ...
       DBGPRINT(RT_DEBUG_TRACE, ("Invalid handle\n"));
    ...
    if(rwContext == NULL) {
       ...
       DBGPRINT(RT_DEBUG_TRACE, ("Failed to alloc mem for rwContext\n"));
    ...
    if(totalLength > BULKUSB_TEST_BOARD_TRANSFER_BUFFER_SIZE) {
       ...
       DBGPRINT(RT_DEBUG_TRACE, ("Transfer length > circular buffer\n"));
    ...
    if(totalLength == 0) {
       ...
       DBGPRINT(RT_DEBUG_TRACE, ("Transfer data length = 0\n"));
    ...
    if(read) {
       ...
       DBGPRINT(RT_DEBUG_TRACE, ("Read operation\n"));
    }
    else {

        urbFlags |= USBD_TRANSFER_DIRECTION_OUT;
       DBGPRINT(RT_DEBUG_TRACE, ("Write operation\n"));
    }
//
    ...
    if(mdl == NULL) {
       ...
       DBGPRINT(RT_DEBUG_TRACE, ("Failed to alloc mem for mdl\n"));
    ...
    if(urb == NULL) {
       ...
       DBGPRINT(RT_DEBUG_TRACE, ("Failed to alloc mem for urb\n"));
   ...
   DBGPRINT(RT_DEBUG_TRACE, ("BulkUsb_DispatchReadWrite::"));
    ...
    if(!NT_SUCCESS(ntStatus)) {
       ...
       DBGPRINT(RT_DEBUG_TRACE, ("IoCallDriver fails with status %X\n", ntStatus));
        ...
        if((ntStatus != STATUS_CANCELLED) && 
           (ntStatus != STATUS_DEVICE_NOT_CONNECTED)) {
            ...
            if(!NT_SUCCESS(ntStatus)) {
               ...
               DBGPRINT(RT_DEBUG_TRACE, ("BulkUsb_ResetPipe failed\n"));
           ...
           DBGPRINT(RT_DEBUG_TRACE, ("ntStatus is STATUS_CANCELLED or "
   ...
   DBGPRINT(RT_DEBUG_TRACE, ("BulkUsb_DispatchReadWrite - ends\n"));
...
--*/
{
    ...
    UNREFERENCED_PARAMETER(DeviceObject);
   DBGPRINT(RT_DEBUG_TRACE, ("BulkUsb_ReadWriteCompletion - begins\n"));
    ...
    if(NT_SUCCESS(ntStatus)) {
        ...
        if(rwContext) {
            ...
            if(rwContext->Length) {
                ...
                //
               DBGPRINT(RT_DEBUG_TRACE, ("Another stage transfer...\n"));
       ...
       DBGPRINT(RT_DEBUG_TRACE, ("ReadWriteCompletion - failed with status = %X\n", ntStatus));
    }
    ...
    if(rwContext) {
        ...
        // dump rwContext
        //
       DBGPRINT(RT_DEBUG_TRACE, ("rwContext->Urb             = %p\n", 
                             rwContext->Urb));
       DBGPRINT(RT_DEBUG_TRACE, ("rwContext->Mdl             = %p\n", 
                             rwContext->Mdl));
       DBGPRINT(RT_DEBUG_TRACE, ("rwContext->Length          = %d\n", 
                             rwContext->Length));
       DBGPRINT(RT_DEBUG_TRACE, ("rwContext->Numxfer         = %d\n", 
                             rwContext->Numxfer));
       DBGPRINT(RT_DEBUG_TRACE, ("rwContext->VirtualAddress  = %X\n", 
                             rwContext->VirtualAddress));
       DBGPRINT(RT_DEBUG_TRACE, ("rwContext->DeviceExtension = %p\n", 
                             rwContext->DeviceExtension));

       DBGPRINT(RT_DEBUG_TRACE, ("BulkUsb_ReadWriteCompletion::"));
   ...
   DBGPRINT(RT_DEBUG_TRACE, ("BulkUsb_ReadWriteCompletion - ends\n"));
