	...
	IN UINT32                DevType)
{
	...
	if(pDevName[devNameLen - 1] >= '0' && pDevName[devNameLen - 1] <= '9')
	{
		preIfIndex = pDevName[devNameLen-1] - 48;
	}
	else
	{
		DBGPRINT(RT_DEBUG_TRACE, ("%s ---> no prefered ifindex(pDevName=%s)\n", __FUNCTION__, pDevName));
		return;
	}
...
/*after p2p cli connect , neet to change to default configure*/
		...
		pAd->SCSCtrl.GoSmCurrState = AP_MAX_AUTH_MSG;/*set to default status when P2P disconnected*/
#endif
} 
