	...
	VOID						*pAdOrg)
{
	...
	/* when enter  function ScanNextChannel, scan should stop immediately and set pAd->Mlme.CntlMachine.CurrState = CNTL_IDLE, 
	otherwise scan trigered by wpa_supplicant  after disconnect finished will be aborted because mlme CntlMachine state is not CNTL_IDLE*/
