...
static void mt7636_switch_channel(RTMP_ADAPTER *pAd, UCHAR channel, BOOLEAN scan)
{
	...
	/* tx pwr gain setting */
	mt7636_tx_pwr_gain(pAd, channel);

	cmd_ch_privilege(pAd, CMD_CH_PRIV_ACTION_REQ, pAd->CommonCfg.Channel, channel, pAd->CommonCfg.BBPCurrentBW,
...
#ifdef RTMP_MAC_PCI
	if (IS_MT7636(ad))
		snprintf((RTMP_STRING) ad->nickname, sizeof(ad->nickname), "mt7636e_sta");
#endif
