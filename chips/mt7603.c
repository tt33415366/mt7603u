...
========================================================================
*/
CHAR ScsGetRssi(IN RTMP_ADAPTER	* pAd)
{
	CHAR	rssi_p2p = SCS_RSSI_ERROR;
	CHAR	rssi_sta = SCS_RSSI_ERROR;
	CHAR	rssi_min = SCS_RSSI_ERROR;
	...
	if (
#ifdef P2P_SUPPORT
	P2P_CLI_ON(pAd) || P2P_GO_ON(pAd) || (pAd->OpMode == OPMODE_AP) ||
#endif /* P2P_SUPPORT */
	...
	FALSE){
	    optype |= SCS_P2P_TYPE;
	    if (pAppAd == NULL) {
	      DBGPRINT(RT_DEBUG_ERROR, ("[SCS]:[%s]:pAppAd == NULL!!!\n", __FUNCTION__));
	      return SCS_RSSI_ERROR;
		  }
		}
	...
	if (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_MEDIA_STATE_CONNECTED)) {
	    optype |= SCS_STA_TYPE;
	    if (pStapAd == NULL) {
	      DBGPRINT(RT_DEBUG_ERROR, ("[SCS]:[%s]:pStapAd == NULL!!!\n", __FUNCTION__));
	      return SCS_RSSI_ERROR;
		  }
		}

	/*p2p is connected*/
	if ((optype & SCS_P2P_TYPE) != 0) {
		rssi_p2p = RTMPAvgRssi(pAd, &pAppAd->RssiSample);
		rssi_p2p -= pAd->BbpRssiToDbmDelta;
		}

	/*sta is connected*/
	if ((optype & SCS_STA_TYPE) != 0) {
		rssi_sta = RTMPAvgRssi(pAd, &pStapAd->RssiSample);
		rssi_sta -= pAd->BbpRssiToDbmDelta;
		}

		switch (optype) {
		case SCS_STA_P2P_MIXED_TYPE:
			rssi_min = (rssi_sta > rssi_p2p)?rssi_p2p:rssi_sta;
			break;
		case SCS_STA_TYPE:
			rssi_min = rssi_sta;
			break;
		case SCS_P2P_TYPE:
			...
			break;
		default:
			DBGPRINT(RT_DEBUG_ERROR, ("[SCS]:[%s]:Get rssi error(optype=%d)!!!\n", __FUNCTION__, optype));
			rssi_min = SCS_RSSI_ERROR;
			break;
	}

  DBGPRINT(pAd->SCSCtrl.SCSDbgLogLv, ("[SCS]:[%s]:optype=%d, rssi_min=%d\n", __FUNCTION__, optype, rssi_min));
  return rssi_min;
}

INT ScsCalTxPer(RTMP_ADAPTER *pAd)
{
	UINT8 per = 0;
	ULONG OneSecTxOkCnt = pAd->SCSCtrl.ScsOneSecTxOkCnt;
	ULONG OneSecTxFailCnt = pAd->SCSCtrl.ScsOneSecTxFailCnt;

	/*When STA Tx-ing, Tx/second count will >SCS_TX_THRESHOLD*/
	if ((OneSecTxOkCnt + OneSecTxFailCnt) > SCS_TX_THRESHOLD) {
		per = (UINT8)(100*(OneSecTxFailCnt)/(OneSecTxFailCnt+OneSecTxOkCnt));
		DBGPRINT(pAd->SCSCtrl.SCSDbgLogLv, ("[SCS]:[%s]:OneSecTxOkCnt=%lu,OneSecTxFailCnt=%lu,per=%d\n", __FUNCTION__, OneSecTxOkCnt, OneSecTxFailCnt, per));
		return per;
		} else {
		DBGPRINT(pAd->SCSCtrl.SCSDbgLogLv, ("[SCS]:[%s]:Tx count < 100\n", __FUNCTION__));
		return -1;
		}
}

INT ScsApNearbyCountByRssi(RTMP_ADAPTER *pAd)
{
	int i;
	BSS_TABLE *Tab = &pAd->SCSCtrl.SCSBssTab;

	if (!Tab) {
		DBGPRINT(RT_DEBUG_ERROR, ("[SCS]:[%s]:SCSBssTab is null!\n", __FUNCTION__));
		return FALSE;
	}

	pAd->SCSCtrl.SCSApCountStage1 = 0;
	pAd->SCSCtrl.SCSApCountStage2 = 0;
	pAd->SCSCtrl.SCSApCountStage3 = 0;

	for (i = 0; i < Tab->BssNr; i++) {
		if ((Tab->BssEntry[i].Rssi < 0) && (Tab->BssEntry[i].Channel == pAd->CommonCfg.Channel)) {
			if (Tab->BssEntry[i].Rssi > -50) {
				pAd->SCSCtrl.SCSApCountStage1++;
			} else if ((Tab->BssEntry[i].Rssi > -70) && (Tab->BssEntry[i].Rssi <= -50)) {
				pAd->SCSCtrl.SCSApCountStage2++;
			} else {
				pAd->SCSCtrl.SCSApCountStage3++;
			}
		}

	}
	DBGPRINT(pAd->SCSCtrl.SCSDbgLogLv, ("[SCS]:[%s]:SCSApCountStage1=%d, SCSApCountStage2=%d, SCSApCountStage3=%d, Tab->BssNr=%d, SCS_status=%d\n", __FUNCTION__,
		pAd->SCSCtrl.SCSApCountStage1, pAd->SCSCtrl.SCSApCountStage2, pAd->SCSCtrl.SCSApCountStage3, Tab->BssNr, pAd->SCSCtrl.SCSStatus));
	return TRUE;
}
...
{
	UINT16 RxRatio = 0;
	CHAR Rssi = SCS_RSSI_ERROR;
	INT per = 0;

	Rssi = ScsGetRssi(pAd);
	if ((pAd->cfg80211_ctrl.FlgCfg80211Scanning == TRUE) || (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS))) {
		DBGPRINT(pAd->SCSCtrl.SCSDbgLogLv, ("[SCS]:[%s]:Force return when scanning\n", __FUNCTION__));
		return FALSE;/*Do nothing when scanning*/
	}
	if ((pAd->RalinkCounters.OneSecReceivedByteCount + pAd->RalinkCounters.OneSecTransmittedByteCount) != 0)
		RxRatio = (UINT16)((pAd->RalinkCounters.OneSecReceivedByteCount) * 100 /
		(pAd->RalinkCounters.OneSecReceivedByteCount +
		pAd->RalinkCounters.OneSecTransmittedByteCount));

	/*Calculate the AP nearby Map by Rssi*/
	ScsApNearbyCountByRssi(pAd);
	/*Calculate the Tx PER per second*/
	per = ScsCalTxPer(pAd);
	pAd->SCSCtrl.ScsOneSecTxOkCnt = 0;
	pAd->SCSCtrl.ScsOneSecTxFailCnt = 0;
	if (per < 0)/*it means sta is not under Tx-ing*/
		return FALSE;

	DBGPRINT(pAd->SCSCtrl.SCSDbgLogLv, ("[SCS]:[%s]:MacTab.Size=%d, BssNr=%d, RSSI=%d, EDCCA=%d, per=%d\n", __FUNCTION__, pAd->MacTab.Size, pAd->SCSCtrl.SCSBssTab.BssNr,
		Rssi, pAd->SCSCtrl.EDCCA_Status, per));

	if ((pAd->MacTab.Size > 0) && (Rssi < 0)) {
		/* Check RSSI for dynamic adjust gain */
		if ((Rssi > -50) && (pAd->SCSCtrl.EDCCA_Status == 0)) {
			if (((pAd->SCSCtrl.SCSApCountStage2+pAd->SCSCtrl.SCSApCountStage3) > SCS_AP_NEARBY_THRESHOLD1) || per > SCS_TX_PER_THRESHOLD1) {
				if (pAd->SCSCtrl.SCSStatus != SCS_STATUS_ULTRA_LOW) {
					...
					RTMP_IO_WRITE32(pAd, CR_CCK_PD_TH, 0x522352B8);
					pAd->SCSCtrl.SCSStatus = SCS_STATUS_ULTRA_LOW;
					DBGPRINT(pAd->SCSCtrl.SCSDbgLogLv,  ("[SCS]:[%s]:SCS=UL\n", __FUNCTION__));
				}
			}
		} else if ((Rssi > -70) && (Rssi <= -50)) {
			if ((pAd->SCSCtrl.SCSApCountStage3 > SCS_AP_NEARBY_THRESHOLD2) || per > SCS_TX_PER_THRESHOLD2) {
				if (pAd->SCSCtrl.SCSStatus != SCS_STATUS_LOW) {
					...
					RTMP_IO_WRITE32(pAd, CR_CCK_PD_TH, 0x522350B8);
					pAd->SCSCtrl.SCSStatus = SCS_STATUS_LOW;
					DBGPRINT(pAd->SCSCtrl.SCSDbgLogLv, ("[SCS]:[%s]:SCS=L\n", __FUNCTION__));
				}
			}
		} else if (Rssi <= -70) {
			if (pAd->SCSCtrl.SCSStatus != SCS_STATUS_DEFAULT) {
				...
				RTMP_IO_WRITE32(pAd, CR_CCK_PD_TH, pAd->SCSCtrl.CR_CCK_PD_TH_default);
				pAd->SCSCtrl.SCSStatus = SCS_STATUS_DEFAULT;
				DBGPRINT(pAd->SCSCtrl.SCSDbgLogLv, ("[SCS]:[%s]:SCS=H (Default)\n", __FUNCTION__));
			}
		}
	} else {
		if (pAd->SCSCtrl.SCSStatus != SCS_STATUS_DEFAULT) {
...
#if 0
INT ScsSetDftGainAtScanBegin(RTMP_ADAPTER *pAd)
{
	if (pAd->SCSCtrl.SCSEnable != SCS_ENABLE)
		return FALSE;

	/*recording current default gain*/
	pAd->SCSCtrl.SCSStatusAtScan = pAd->SCSCtrl.SCSStatus;
	DBGPRINT(RT_DEBUG_TRACE, ("%s(): SCSStatusAtScan =%d,SCSStatus=%d\n", __FUNCTION__, pAd->SCSCtrl.SCSStatusAtScan, pAd->SCSCtrl.SCSStatus));
	/*set to high gain*/
	if (pAd->SCSCtrl.SCSStatus != SCS_STATUS_DEFAULT) {
		RTMP_IO_WRITE32(pAd, CR_AGC_0, pAd->SCSCtrl.CR_AGC_0_default);
		RTMP_IO_WRITE32(pAd, CR_AGC_0_RX1, pAd->SCSCtrl.CR_AGC_0_default);
		...
		RTMP_IO_WRITE32(pAd, CR_CCK_PD_TH, pAd->SCSCtrl.CR_CCK_PD_TH_default);
		pAd->SCSCtrl.SCSStatus = SCS_STATUS_DEFAULT;
		DBGPRINT(RT_DEBUG_TRACE, ("%s(): SCS=H (Default)\n", __FUNCTION__));
	}
	return TRUE;
}

INT ScsRestoreLastGainAtScanEnd(RTMP_ADAPTER *pAd)
{
	if (pAd->SCSCtrl.SCSEnable != SCS_ENABLE)
		return FALSE;

	/*Restore to last gain*/
	DBGPRINT(RT_DEBUG_TRACE, ("%s(): SCSStatusAtScan =%d,SCSStatus=%d\n", __FUNCTION__, pAd->SCSCtrl.SCSStatusAtScan, pAd->SCSCtrl.SCSStatus));
	if (pAd->SCSCtrl.SCSStatusAtScan != pAd->SCSCtrl.SCSStatus) {
		switch (pAd->SCSCtrl.SCSStatusAtScan) {
		case SCS_STATUS_DEFAULT:
				RTMP_IO_WRITE32(pAd, CR_AGC_0, pAd->SCSCtrl.CR_AGC_0_default);
				RTMP_IO_WRITE32(pAd, CR_AGC_0_RX1, pAd->SCSCtrl.CR_AGC_0_default);
				RTMP_IO_WRITE32(pAd, CR_AGC_3, pAd->SCSCtrl.CR_AGC_3_default);
				RTMP_IO_WRITE32(pAd, CR_AGC_3_RX1, pAd->SCSCtrl.CR_AGC_3_default);
		...
		case SCS_STATUS_LOW:
				...
				RTMP_IO_WRITE32(pAd, CR_CCK_PD_TH, 0x522350B8);
				pAd->SCSCtrl.SCSStatus = SCS_STATUS_LOW;
				DBGPRINT(RT_DEBUG_TRACE, ("%s(): SCS=L\n", __FUNCTION__));
				break;

		case SCS_STATUS_ULTRA_LOW:
				RTMP_IO_WRITE32(pAd, CR_AGC_0, 0x7FF6666F);
				RTMP_IO_WRITE32(pAd, CR_AGC_0_RX1, 0x7FF6666F);
				RTMP_IO_WRITE32(pAd, CR_AGC_3, 0x818181E3);
				RTMP_IO_WRITE32(pAd, CR_AGC_3_RX1, 0x818181E3);
				RTMP_IO_WRITE32(pAd, CR_CCK_PD_TH, 0x522352B8);
				pAd->SCSCtrl.SCSStatus = SCS_STATUS_ULTRA_LOW;
				DBGPRINT(RT_DEBUG_TRACE,  ("%s(): SCS=UL\n", __FUNCTION__));
				break;

		default:
				DBGPRINT(RT_DEBUG_TRACE, ("%s(): SCS error at scan stage!\n", __FUNCTION__));
				break;
		}
	}
	return TRUE;
...
INT ScsQuickCheckToSetDefGain(RTMP_ADAPTER *pAd)
{
	BOOLEAN IsScanning = FALSE;
	BOOLEAN IsConnecting = FALSE;
#ifdef RT_CFG80211_SUPPORT
	BOOLEAN P2pGoNeedSetDefGain = FALSE;/*P2P GO*/
	ULONG   P2pGoCurrState = pAd->SCSCtrl.GoSmCurrState;
#endif

	if (pAd->SCSCtrl.SCSEnable != SCS_ENABLE)
		return TRUE;

	/*STA or P2P is scanning/listening*/
	if ((pAd->cfg80211_ctrl.FlgCfg80211Scanning == TRUE) || (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS)) || (pAd->cfg80211_ctrl.Cfg80211RocTimerRunning)) {
			DBGPRINT(pAd->SCSCtrl.SCSDbgLogLv, ("[SCS]:[%s]:Scanning!!!\n", __FUNCTION__));
			IsScanning = TRUE;
		}

	/*STA or P2P-GC is connecting*/
	if (pAd->cfg80211_ctrl.FlgCfg80211Connecting == TRUE) {
			DBGPRINT(pAd->SCSCtrl.SCSDbgLogLv, ("[SCS]:[%s]:Connecting!!!\n", __FUNCTION__));
			IsConnecting = TRUE;
		}

	/*P2P-GO is connecting*/
#ifdef RT_CFG80211_SUPPORT
	if ((P2pGoCurrState == APMT2_PEER_ASSOC_REQ) || (P2pGoCurrState == APMT2_PEER_REASSOC_REQ) || (P2pGoCurrState == APMT2_PEER_AUTH_REQ)) {
			DBGPRINT(pAd->SCSCtrl.SCSDbgLogLv, ("[SCS]:[%s]:P2pGoCurrState = %lu\n", __FUNCTION__, P2pGoCurrState));
			P2pGoNeedSetDefGain = TRUE;
		}
#endif
