	...
	*/
	...
	if (bScan == FALSE) {
		if (strncmp(pAd->CommonCfg.CountryCode, "FR", 2) == 0)
		{
			...
			DBGPRINT(RT_DEBUG_OFF, ("%s():Enable ED CCA Check for cert!\n", __FUNCTION__));
		} else {
			RT6352_set_ed_cca(pAd, FALSE);
			DBGPRINT(RT_DEBUG_OFF, ("%s():Disable ED CCA Check!\n", __FUNCTION__));
		}
	}
#endif /* ED_CERT */
...
redo:
	...
	for(ant_idx = 0; ant_idx < 2; ant_idx = ant_idx + 1){
		...
		for (index = 0; index < 4; index++)
		{
			...
			DBGPRINT(RT_DEBUG_INFO, ("RF_B4R39 %08x\n", RFValue));
			RT635xReadRFRegister(pAd, RF_BANK6, RF_R39, &RFValue);
			DBGPRINT(RT_DEBUG_INFO, ("RF_B6R39 %08x\n", RFValue));
			...
			DBGPRINT(RT_DEBUG_INFO, ("BBP_R244 is 0x%02x\n", BBPValue));
			RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R22, &BBPValue);
			DBGPRINT(RT_DEBUG_INFO, ("BBP_R22 is 0x%02x\n", BBPValue));
#if 0
			BBPValue = 0x50;
			while (BBPValue == 0x50)
			{
			...
			DBGPRINT(RT_DEBUG_INFO, ("BBP_R47 is 0x%02x\n", BBPValue));
#else
			...
			if(ant_idx == 0){
				switch (index){
					...
					case 1: 								
						...
						DBGPRINT(RT_DEBUG_INFO, ("rt635x_tssi0_dc_hvga is 0x%08x\n", rt635x_tssi0_dc_hvga));
						break;
					case 2:								
						tssi0_linear = (INT32)BBPR49;
						DBGPRINT(RT_DEBUG_INFO, ("index %d: tssi0_linear is 0x%08x\n", index, tssi0_linear));
								
						tssi0_db = MT76xx_lin2dBd(tssi0_linear - rt635x_tssi0_dc);
						DBGPRINT(RT_DEBUG_INFO, ("tssi0_db is %d\n", tssi0_db));
						break;
					case 3:								
						tssi0_linear = (INT32)BBPR49;
						DBGPRINT(RT_DEBUG_INFO, ("index %d: tssi0_linear hvga is 0x%08x\n", index, tssi0_linear));
				...
				switch (index){
					...
					case 1:					
						...
						DBGPRINT(RT_DEBUG_INFO, ("rt635x_tssi1_dc_hvga is 0x%08x\n", rt635x_tssi1_dc_hvga));
						break;
					case 2:
						tssi1_linear = (INT32)BBPR49;
						DBGPRINT(RT_DEBUG_INFO, ("index %d: tssi1_linear is 0x%08x\n", index, tssi1_linear));
						
						tssi1_db = MT76xx_lin2dBd(tssi1_linear - rt635x_tssi1_dc); 			
						DBGPRINT(RT_DEBUG_INFO, ("tssi1_db is %d\n", tssi1_db));
						break;
					case 3:				
						tssi1_linear = (INT32)BBPR49;
						DBGPRINT(RT_DEBUG_INFO, ("index %d: tssi1_linear hvga is 0x%08x\n", index, tssi1_linear));
...
#endif
						DBGPRINT(RT_DEBUG_INFO, ("tssi1_db_hvga is %d\n", rt635x_tssi1_db_hvga));						
						break;
				}
			}
...
//	    if ((base_power[chain] + cur_comp_power) >= 0)
	...
	pa_mode_used = (pa_mode == 0x00) ? pa_mode_00 :
		...
		DBGPRINT(RT_DEBUG_INFO, ("rt635x_tssi0_dc %x\n", rt635x_tssi0_dc));
		DBGPRINT(RT_DEBUG_INFO, ("rt635x_tssi1_dc %x\n", rt635x_tssi1_dc));
		DBGPRINT(RT_DEBUG_INFO, ("rt635x_tssi0_dc_hvga %x\n", rt635x_tssi0_dc_hvga));
		DBGPRINT(RT_DEBUG_INFO, ("rt635x_tssi1_dc_hvga %x\n", rt635x_tssi1_dc_hvga));
		DBGPRINT(RT_DEBUG_INFO, ("tssi_dc %x\n", tssi_dc));
		DBGPRINT(RT_DEBUG_INFO, ("tssi_linear %x\n", tssi_read[chain]));
		...
		DBGPRINT(RT_DEBUG_INFO, ("tssi_slope  %x\n", rt635x_tssi_slope[chain]));
		DBGPRINT(RT_DEBUG_INFO, ("tssi offset %x\n", rt635x_tssi_offset[ch_group][chain]));
		DBGPRINT(RT_DEBUG_INFO, ("\npwr is %d \n", pwr));
		...
		DBGPRINT(RT_DEBUG_INFO, ("mcs_power      %8d\n", mcs_power));
		DBGPRINT(RT_DEBUG_INFO, ("pkt_type_delta %8d\n", pkt_type_delta));
		DBGPRINT(RT_DEBUG_INFO, ("pa mode        %8d\n", pa_mode));
		DBGPRINT(RT_DEBUG_INFO, ("pa mode used   %8d\n", pa_mode_used));
		...
		DBGPRINT(RT_DEBUG_INFO, ("target_power %d, pwr %d\n", target_power, pwr));
		...
		DBGPRINT(RT_DEBUG_INFO, ("temp[%d] is 0x%02x\n", chain, temp[chain]));
		...
		DBGPRINT(RT_DEBUG_INFO, ("pwr_diff is %d\n", pwr_diff[chain]));
		DBGPRINT(RT_DEBUG_INFO, ("_comp_power is 0x%08x\n", _comp_power));
		if (chain == 0)
		{
			if (_comp_power > 31)
				_comp_power = 31;
		...
		DBGPRINT(RT_DEBUG_INFO, ("comp_power[%d] is 0x%08x\n", chain, comp_power[chain]));
	}
	...
	DBGPRINT(RT_DEBUG_INFO, ("comp_power[0] is 0x%08x\n", comp_power[0]));
	if (max_stream == 2)
		DBGPRINT(RT_DEBUG_INFO, ("comp_power[1] is 0x%08x\n", comp_power[1]));
	...
	DBGPRINT(RT_DEBUG_INFO, ("\n================================\n"));
	DBGPRINT(RT_DEBUG_INFO, ("Mac 0x13B4 is 0x%08x\n", MacValue));
	RTMP_IO_WRITE32(pAd, TX_ALG_CFG_1, MacValue);	
	...
	DBGPRINT(RT_DEBUG_INFO, ("Mac 0x13B0 is 0x%08x\n", MacValue));
	RTMP_IO_WRITE32(pAd, TX_ALG_CFG_0, MacValue);
...
#ifdef RELEASE_EXCLUDE
			DBGPRINT(RT_DEBUG_TRACE, ("%s::after compensation, offset = 0x%04X, register value = 0x%08lX\n",
