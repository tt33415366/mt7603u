		...
		if (MlmeDequeue(&pAd->Mlme.Queue, &Elem)) 
		{
			...
			switch (Elem->Machine) 
			{
				...
				case AP_ASSOC_STATE_MACHINE:
					...
					pAd->SCSCtrl.GoSmCurrState = Elem->MsgType;
#endif
...
/*	RECBATimerTimeout(SystemSpecific1,FunctionContext,SystemSpecific2,SystemSpecific3);*/
...
#ifdef SMART_CARRIER_SENSE_SUPPORT
	if (ScsQuickCheckToSetDefGain(pAd) == TRUE)
		ScsAvailable = FALSE;
#endif
	...
	*/
		...
		if ((OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_MEDIA_STATE_CONNECTED)
				...
				|| RTMP_CFG80211_VIF_P2P_GO_ON(pAd) || RTMP_CFG80211_VIF_P2P_CLI_ON(pAd)
#endif /*RT_CFG80211_SUPPORT*/
		) && (pAd->SCSCtrl.SCSEnable == SCS_ENABLE) && (ScsAvailable == TRUE)) {
			...
			if (pAd->SCSCtrl.SCSStatus != SCS_STATUS_DEFAULT) {
				...
				DBGPRINT(RT_DEBUG_TRACE, ("No connection, set CSC=H (Default)\n"));
			}
		}
