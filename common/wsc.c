	...
	switch (MsgType)
	{
		...
		case WSC_MSG_M2D:
				...
				pWscControl->WscState == WSC_STATE_RX_M2D)
			{
				...
				if ((CurOpMode == STA_MODE)
#ifdef P2P_SUPPORT
					 || (P2P_INF_ON(pAdapter) && (P2P_CLI_ON(pAdapter))) /* In P2P Case : CurOpMode is always AP_MODE, So we need use P2P_CLI_ON() to check*/
#endif
					)
				{
...
VOID WscWriteConfToDatFile(RTMP_ADAPTER *pAd, UCHAR CurOpMode)
{
	...
	else 
	{
		...
		if (cfgData == NULL)
		{
			...
			DBGPRINT(RT_DEBUG_ERROR, ("mem alloc fail. (fileLen = %ld)\n", fileLen));
			goto out;
		}
	...
	if (!profileTemp)
	{
		DBGPRINT(RT_DEBUG_ERROR, ("profileTemp mem alloc fail. (255)\n"));
		goto WriteErr;
	}
	else
		DBGPRINT(RT_DEBUG_ERROR, ("profileTemp mem alloc succeed. (255)\n"));
