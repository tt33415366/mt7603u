	...
	IN PRTMP_ADAPTER pAd)
{
...
#ifdef RELEASE_EXCLUDE
	DBGPRINT(RT_DEBUG_INFO, ("\t\tCheck CAL_R0(Cnt:%d, Value:0x%x)\n",
			cnt, bbp_val));
#endif /* RELEASE_EXCLUDE */
	...
	if (cal_val >= 0x40)
		cal_val -= 128;
#ifdef RELEASE_EXCLUDE
	DBGPRINT(RT_DEBUG_INFO, ("\t\tCAL_R57:0x%x => cal_val=0x%02x\n",
...
do_cal:
...
#ifdef RELEASE_EXCLUDE
		DBGPRINT(RT_DEBUG_INFO, ("\t\tR57(Init=%d,Cal=%d,Diff=%d,FilterTarget=%d,",
							cal_r32_init, cal_r32_val, cal_diff, filter_target));
		if (bTxCal)
			DBGPRINT(RT_DEBUG_INFO, ("TxAgcFc=%d)\n", tx_agc_fc));
		else
			DBGPRINT(RT_DEBUG_INFO, ("RxAgcFc=%d)\n", rx_agc_fc));
#endif /* RELEASE_EXCLUDE */
			...
			((cal_diff < filter_target) && (cmm_agc_fc == 0x3f)))
		{
			...
			if (bTxCal)
				...
				DBGPRINT(RT_DEBUG_INFO, ("\t\tBW%dM FilterCal cannot find valid val(loop=%d), set RxAgcFc=0\n",
								(loop == 0 ? 20 : 40), loop));
#endif /* RELEASE_EXCLUDE */
			if (bTxCal)
				tx_agc_fc = 0;
...
#ifdef RELEASE_EXCLUDE
			if (bTxCal)
				DBGPRINT(RT_DEBUG_INFO, ("\t\ttx_agc_fc++ = %d, redo cal!\n", tx_agc_fc));
			else
				DBGPRINT(RT_DEBUG_INFO, ("\t\trx_agc_fc++ = %d, redo cal!\n", rx_agc_fc));
#endif /* RELEASE_EXCLUDE */
			goto do_cal;
		}
		else
		{
			...
			if (bTxCal)
				...
				DBGPRINT(RT_DEBUG_INFO, ("\t\tget rx_agc_fc, cal done!\n"));
#endif /* RELEASE_EXCLUDE */
		}
	
#ifdef RELEASE_EXCLUDE
		if (bTxCal)
			DBGPRINT(RT_DEBUG_ERROR, ("\t=>BW %dMHz FilterCal Value=0x%x!\n",
