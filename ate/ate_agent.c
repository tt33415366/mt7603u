...
CHAR TemplateFrame[32] = {0x08, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF,
	...
	RTMP_STRING *Arg)
{
	...
	else if (!strcmp(Arg, "RXSTOP") && (ATECtrl->Mode & ATE_START))
	{
	...
	} else if (!strcmp(Arg, "TXCARS") && (ATECtrl->Mode & ATE_START)) {
		INT32 pwr1 = 0xf;
		INT32 pwr2 = 0;
		/* 0: All 1:TX0 2:TX1 */
		switch(ATECtrl->TxAntennaSel){
		case 0:
			DBGPRINT(RT_DEBUG_OFF,
				("%s: not support two 2 TXCARRSUPP\n", __func__));
			break;
		case 1:
			...
			DBGPRINT(RT_DEBUG_OFF,
				("%s: TXCARRSUPP, TxPower0:0x%x, pwr1:0x%x, pwr2:0x%x\n",
					__func__, ATECtrl->TxPower0, pwr1, pwr2));
			break;
		case 2:
			...
			DBGPRINT(RT_DEBUG_OFF,
				("%s: TXCARRSUPP, TxPower1:0x%x, pwr1:0x%x, pwr2:0x%x\n",
					__func__, ATECtrl->TxPower1, pwr1, pwr2));
			break;
		}
